 ...
 * - Limited to approximately 30 FPS
 * - Reduced CPU/GPU usage for smoother performance
 */
export default function ThreeDCanvas({ className = '' }) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      targetRotY =
        ((e.clientX - halfW) / halfW) * 0.8;

      targetRotX =
        -((e.clientY - halfH) / halfH) * 0.8;
    };
    ...
    /* ---------------------------------------------------------
       2. Define 3D Geometric Polyhedron Nodes
       --------------------------------------------------------- */
    ...
    for (
      let i = 0;
      i < coreVertices.length;
      i++
    ) {
      for (
        let j = i + 1;
        j < coreVertices.length;
        j++
      ) {
        const dx =
          coreVertices[i].x -
    ...
    const render = () => {
      ...
      /* Smooth mouse follow */
      rotX +=
        (targetRotX - rotX) * 0.05;

      rotY +=
        (targetRotY - rotY) * 0.05;
