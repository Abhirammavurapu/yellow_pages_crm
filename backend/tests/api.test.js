...
test('API 3: Concurrency Locking Workflow via HTTP APIs', async () => {
  ...
  // Release any existing lock on target lead
  ...
  // Caller 1 acquires lock
  const lockRes1 = await apiRequest(
    `/leads/${targetLeadId}/lock`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token1}`
  ...
  // Caller 2 attempts to lock the SAME lead
  const lockRes2 = await apiRequest(
    `/leads/${targetLeadId}/lock`,
    {
      method: 'POST',
      headers: {
  ...
  assert.strictEqual(
    lockRes2.status,
    423,
    'Simultaneous lock must return HTTP 423'
  );

  assert.strictEqual(
    lockRes2.data.errorCode,
    'LEAD_LOCKED'
  );

  assert.ok(
    lockRes2.data.message.includes('Anita Desai'),
    'Message must name current caller'
  );

  // Caller 1 logs a call
  const callRes = await apiRequest(
    `/calls/lead/${targetLeadId}`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token1}`
      ...
      body: {
        ...
        duration: 180,
        notes:
          'Customer confirmed interest in Yellow Pages premium listing',
        followUpDate:
          new Date(
            Date.now() + 86400000
          ).toISOString(),
  ...
  // Caller 2 can now acquire the lock
  const lockRes2Retry = await apiRequest(
    `/leads/${targetLeadId}/lock`,
    {
      method: 'POST',
      headers: {
  ...
  assert.strictEqual(
    lockRes2Retry.status,
    200,
    'Caller 2 can now acquire lock after Caller 1 completed call'
  );
...
test('API 4: Location Hierarchy Endpoints', async () => {
  ...
  const districtsRes = await apiRequest(
...
test('API 5: Public User Sign Up and Direct Authentication', async () => {
  ...
  assert.strictEqual(
    signupRes.data.data.user.role,
    'TELECALLER'
  );
  assert.ok(
    signupRes.data.data.token,
    'Must return JWT token for immediate login'
  );
});

test('API 6: Admin Registration with Security Passkey', async () => {
  const randomAdminEmail =
  ...
  assert.strictEqual(
    successRes.data.data.user.role,
    'ADMIN'
  );
...
test('API 7: Lead Creation Auto-Assignment & Queue Visibility', async () => {
  ...
  assert.strictEqual(
    leadRes.data.data.assignedTo,
    callerId,
    'Lead must be auto-assigned to creator'
  );
  ...
  const found = myLeads.data.data.some(
    (l) =>
      l.businessName === 'Apex Precision Tools' ||
      (
        l.phoneNumbers &&
        l.phoneNumbers.includes(uniquePhone)
      )
  );
  ...
  assert.strictEqual(
    found,
    true,
    'Lead must be present in caller leads list'
  );
});
...
test('API 8: Duplicate Phone Prevention', async () => {
  ...
  assert.strictEqual(
    second.status,
    409,
    'Duplicate phone must be rejected with 409'
  );
});